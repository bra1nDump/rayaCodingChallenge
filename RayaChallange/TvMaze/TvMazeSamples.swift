...
#if DEBUG
enum TvMazeSamples {
    static let searchShows = try! JSONDecoder().decode(TvMaze.ShowSearchMatches.self, from: searchShowsData)
    static let searchShowsData =
        """
        [
          {
            "score": 17.261711,
    ...
    static let episodes = try! JSONDecoder().decode(TvMaze.Episodes.self, from: episodesData)
    static let episodesData = #"""
        [
          {
            "id": 1,
